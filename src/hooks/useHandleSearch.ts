...
import { Match, Competition, Team } from '../types/types';
...
const useHandleSearch = (competitions: Competition[]) => {
  ...
  const { teams, isLoading: isLoadingTeams, error: teamsError } = useFetchTeams(championshipId);
  const { matches, isLoading: isLoadingMatches, error: matchesError } = useFetchMatches(championshipId);
  ...
  useEffect(() => {
    if (championshipId) {
      const selectedComp = competitions.find(comp => comp.id === championshipId);
      setSelectedCompetition(selectedComp ? selectedComp.name : '');

      const uniqueRounds = [...new Set(matches.map(match => match.matchday))].sort((a, b) => a - b);
  ...
  useEffect(() => {
    ...
    if (round && round !== 'all') {
      filtered = filtered.filter(match => match.matchday.toString() === round);
    }

    setFilteredMatches(filtered);
  }, [teamName, round, matches]);

  const handleSearch = (selectedChampionship: string, team: string, round: string) => {
